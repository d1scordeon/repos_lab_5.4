...
double S0(const int K, const int N)
{
    double sum = 0;
    for (int i = K; i <= N; i++)
        sum += 1. * pow(i, 2) / (1. * pow(K, 2) + pow(N, 2));
    return sum;
}
double S1(const int K, const int N, const int i)
{
    if (i > N)
        ...
        return (1. * pow(i, 2) / (1.0 * pow(K, 2) + pow(N, 2))) + S1(K, N, i + 1);
}
double S2(const int K, const int N, const int i)
{
    if (i < K)
        ...
        return (1. * pow(i, 2) / (1. * pow(K, 2) + pow(N, 2))) + S2(K, N, i - 1);
}
double S3(const int K, const int N, const int i, double t)
{
    t = t + (1. * pow(i, 2) / (1. * pow(K, 2) + pow(N, 2)));
    if (i >= N)
        return t;
    else
        return S3(K, N, i + 1, t);
...
{
    t = t + (1.* pow(i, 2) / (1.0 * pow(K, 2) + pow(N, 2)));
    if (i <= K)
        return t;
    else
        return S4(K, N, i - 1, t);
